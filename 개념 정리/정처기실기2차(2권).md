...
**메모리 종류 접근 속도 고속 순서** - 레지스터 > 캐시 메모리 > 주기억장치 > 가상메모리

**가상화** - 컴퓨터 자원의 추상화 / 물리적 자원들을 사용자에게 하나로 보이게 하거나, 하나를 여러 개로 보이게 하는

**클라우드 컴퓨팅** - 인터넷을 통해 가상화된 컴퓨터 시스템 자원을 요구하는 즉시 처리하여 제공

- IaaS(Infrastructure as a Service) - 서버, 스토리지 자원을 요구하는 즉시 처리해서 제공
- PaaS(Platform as a Service) - 서비스를 개발할 수 있는 안정적인 환경과, 응용 프로그램을 개발할 수 있는 API까지 제공
- SaaS(Software as a Service) - 주문형 소프트웨어 / 단말기 등에서 필요하면 언제든지 서비스 받을 수 ㅇ
...
**데이터베이스** - 어느 한 조직에서 업무 처리 위해 통합, 저장된 운영 데이터의 집합

- 종류
  - 통합된 데이터 - 중복 최소화
  - 저장된 데이터 - 필요시 언제든지 이용할 수 있도록
  - 공용 데이터 
  - 운영 데이터 - 이용가치가 있는 데이터의 집합
- 특징
  - 계속적인 변화 - 항상 최신 정보 유지할 수 있도록
  - 동시 공유 - 여러 사용자가 동시에 접근
  - 실시간 접근성 - 질의에 대해 실시간 처리 및 응답
  - 내용에 의한 참조 - 요구하는 데이터 내용으로 검색
...
**DBMS** - 데이터의 중복성과 종속성 등의 문제 최소화 위해 / 사용자와 데이터베이스 간 중계 역할

- 필수 기능
  - 정의 - db 저장에 관한 여러 가지 사항을 정의
  - 조작 - 자료를 사용자가 이용할 수 있도록 검색, 갱신, 삽입 등 지원
  - 제어 - 데이터 무결성 유지, 보안, 병행 수행 제어 등 제공
    - 무결성 : 자료가 오류없이 정확성과 안전성을 유지하기 위한 제약 조건이나 성질
- 언어
...
**DB 표현** - 논리적 구조(사용자 관점) / 물리적 구조(기계 관점)

**스키마** - 데이터베이스의 전체적인 구조와 제약조건에 대한 명세를 기술, 정의

- 내부 스키마 - 물리적 저장 장치 관점
- 개념 스키마 - 논리적 관점
- 외부 스키마 - 사용자가 사용하는 한 부분에서 본 논리적 구조(서브 스키마)

**데이터베이스 관리자(DBA)** - 모든 자원에 대해 기획, 통제 / 전체적인 관리 운영에 책임을 지는 사람이나 집단
...
1. sprint planning meeting - 단기 일정 수립
2. sprint - 실제 개발(약 2주)
3. daily scrum meeting - 매일 약 15분 / 남은 작업시간 **소멸차트**에 표시
4. sprint review - 테스팅
5. sprint retrospective - 규칙 준수 여부, 개선점



scrum prodeuct owner가 **backlog**작성 (개발에 필요한 요구사항 다 모아서 우선순위 부여한 목록)



XP

- communication / simplicity / courage / respect / feedback
- 개발 프로세스
  1. release planning - 일정 수립
  2. iteration - 실제 개발(보통 1 ~ 3주)
  3. aceptance test - 인수 테스트 (부분 완료 제품 구현되면 테스트)
  4. small release - 릴리즈 규모 축소
- 실천 방법
  - pair programming
  - collective ownership
  - test driven development - 테스트 케이스를 먼저 작성해야 뭘 해야 할 지 앎
  - whole team
  - continuous integration - 모듈 단위 완료되면 통합
  - refactoring - 기능 변경 없이 시스템 재구성 / 쉽게 이해하고 수정 빨리하기 위해
  - small releases
...
WAS(Web Application Server) - 동적 컨텐츠 처리 위해 사용되는 **미들웨어**



요구사항 개발 프로세스 - Elicitation > Analysis > Specification > Validation(형상관리 수행)



DFD - process / data flow / data store / terminator



UML - 의사소통 원활하게 이뤄지도록 표준화한 객체지향 모델링 언어
...
- Behavioral(유시컴상활타)
  - usecase - 사용자 요구 분석
  - sequence - 상호작용하는 객체들 주고받는 메세지
  - communication - 메세지와 연관관계 포함
  - state - 럼바우에서 동적 모델링
  - activity - 처리흐름 순서 따라
  - interaction overview
  - timing

기능 모델링 - usecase / activity

정적 모델링 - class / package

동적 모델링 - sequence / communication / state



객체지향

- 구성요소 - object / message / class
- 기본원칙 - 캡슐화 / 정보은닉 / 추상화 / 상속성 / 다형성



embedded software - 특정 기능에 맞게 특화돼서 제품 자체에 포함된 소프트웨어
...
- 하향식
  - 전문가 감정 기법 - 경험 많은 두 명 이상 전문가에게 의뢰
  - 델파이 기법 - 많은 전문가에게 의뢰
- 상향식
  - LOC - 비관치, 낙관치, 중간치 통해 예측치 측정
  - 개발단계별 인원수 - loc보다 정확
  - 수학적 산정 - 자동화 목표 (COCOMO / PUTNAM / FP 기능점수 모형)
...
- PERT - 비관치, 중간치, 낙관치
- CPM - 주 공정법 / 노드 간 연결 통해 공정 계산 / 액티비티 표기법 / 최장경로(**임계경로**)
- CCPM - 제약사항 고려해서 일정 작성(중요 연쇄 프로젝트 관리)
- WBS - 프로젝트 n개 단위로 분할

간트차트 - 언제 시작하고 언제 종료되는지 막대도표로 표시



CMMI - 카네기멜론 대학교 / 조직 성숙도

- initial > managed > defined > 정량적 관리 > optimizing(최적화)

ISO/IEC 15504 = SPICE



소프트웨어 개발 프레임워크 - 구성요소와 아키텍쳐를 일반화 시켜서 손쉽게 구현하도록
