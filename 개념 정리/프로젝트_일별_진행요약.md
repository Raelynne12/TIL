...
아이디 컬럼이 없는데 어떻게 통일시켜줘야 하는지

아이템 고유 개수가 다름 >> 이걸 어떻게 맞출지



마이너스값은 판매가 된 적이 없다는 뜻이래



원핫 인코딩 / 평균 인코딩(연말은 개수가 다르니까 가중치를 다르게 해서 하는 방법)



샵에 따른 아이템별 가격(shop_cnt_month)
...
- 보니까 랜덤 포레스트랑 부스팅, 리니어(선형 회귀)를 많이 사용함
...
파워비아이로 만들어봤대 그래서 시각화로 한 번 해보고 분석해봤대