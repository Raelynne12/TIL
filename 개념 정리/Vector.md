...
- 내적
  ...
  - 두 벡터의 차원이 같아야 함(앞 벡터 열개수 = 뒤 벡터가 행개수)
  - 앞 벡터가 행벡터, 뒤 벡터 열벡터
...
\- ex) 최근접 이웃 사용 시 데이터 간 거리 측정하고 가장 가까운 벡터끼리 그룹 형성
...
>>해설<<
A(1,2,3) B2(1,3,2) << 차원
첫 번째 배열의 마지막 축(3)과 두 번째 배열의 뒤에서 두 번째 축(3)의 크기가 같아야 하기 때문에 B2만 내적 가능
```
...
- 외적
  ...
  - 벡터곱(vector product)
  - 3차원 벡터의 외적 결과가 3차원일 때 벡터곱
  - 외적 결과가 행렬 >> 외적(outer product)
  - 내적과 달리 연산 결과가 벡터
    - 방향 : 두 벡터에 동시에 수직
    - 크기 : 두 벡터 크기를 변으로 하는 평행사변형 넓이
  - ![image-20220109142047414](Vector.assets/image-20220109142047414.png)
  - 외적에서는 내적의 교환법칙 성립 안 함
...
> SVM(데이터가 어떤 범주에 속할지 판별하는 분류 모델) >> 패턴 인식이나 자료 분석에 주로 사용
>
> \- SVM 원리: 마진을 최대로하는 결졍 경계 찾기
>
> ​					 결정 경계 찾을 때 사용하는 게 벡터 **직교와 내적**



![image-20220109144011785](Vector.assets/image-20220109144011785.png)

결정 경계랑 직교하는 w, u 벡터

w, u 벡터를 내적한 결과가 임의의 상수 k보다 큰 지 확인

만약 내적한 게 >= k 면 동그라미 범주에 속하고, 그렇지 않다면 네모 범주에 속함
