...
sample = sample_1.append(sample_2_code,
                        ignore_index = True)	#true로 안하면 번호 012340123이런식
sample
```
...
크롬 버전 확인 >> 자기 버전에 맞는 크롬드라이버 다운로드

웹페이지 접속 >> url >> `driver.get(url)`

`html =  driver.page_source`  : 웹페이지의 html 다운로드 >> html에서 원하는 정보 크롤링
